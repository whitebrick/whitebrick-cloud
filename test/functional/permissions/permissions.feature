Feature: Permissions
  ...
  Scenario: Adding new organization admin should implicitly assign schema admin to organization-owned schemas
    ...
    * table schemaUsers 
      ...
      | "test_donna@test.whitebrick.com" | "test_org_admins" | ["test_nick_north@test.whitebrick.com"]
  ...
  Scenario: Removing an organization admin should remove implicitly assigned schema admins
    ...
    * table schemaUsers 
      | currentUserEmail                 | schemaName        | userEmails
      | "test_donna@test.whitebrick.com" | "test_org_admins" | ["test_daisy@test.whitebrick.com"]
    * def result = call read("../schemas/schema-get-users.feature") schemaUsers
    * match each result[*].response contains { errors: "#notpresent" }
